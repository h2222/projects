...
from config import boddy, prem_path, vocab_path, url_sort, params_sort_027, h_sort,in_node_sort, url_cls, params_cls, header_cls, h_cls
...
def test():
    ...
    if request.method=='POST':
        ...
        params_sort_027['bizParam']['inparam']['msg'] = json_data['word']
        params_sort_027['bizParam']['inparam']['in_node'] = json_data['currentNodeId']
        params_sort_027['bizParam']['inparam']['processid'] = json_data['id']
        ...
        # 建立事件循环
        # tasks = [models(url_sort, params_sort_027, None), models(url_cls, param_cls, header_cls)]
        # 只保留分类模型
        tasks = [models(url_cls, param_cls, header_cls)]
        # 执行事件循环并返回结果
        ...
        if len(dataset) == 1 and 'predictions' in dataset[0]:
            dataset = {'cls_result':dataset[0]}
        # 因为异步, 判断dataset前后谁是sort谁是cls
        elif 'predictions' in dataset[0]:
        ...
        # 判断相似度是否大于阈值
        try:
            sim = float(dataset['sort_result']['resultData']['semantic_analysis_new_V0_27']['similarity'])
        except:
            # 命中规则, sim等于1
            sim = 1
        if sim >= 0.8 and 'sort_result' in dataset:
            outnode_sort = dataset['sort_result']['resultData']['semantic_analysis_new_V0_27']['out_node']
...
if __name__ == "__main__":
    app.run(host='0.0.0.0', port=10091, debug=True)
