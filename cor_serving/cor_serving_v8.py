...
from config import boddy, prem_path, vocab_path, url_sort, params_sort, h_sort, url_cls, params_cls, header_cls, h_cls
from others import (PreProcessing, FullTokenizer)

app = Flask(__name__)
CORS(app,  resources={r"/*": {"origins": "*"}})
tokenizer = FullTokenizer('./others/vocab.txt')
pre_processing = PreProcessing(tokenizer)
...
def test():
    ...
    # 分类模型, 获取word
    x = pre_processing.deal_combine(json_data['word'])
    ...
    if sim >= 0.8 :
        ...
        if json_data['currentNodeId'] == '6468':
            prediction = dataset['cls_result']['predictions'][0]['identity1']
            out_nodes = {'yes':'6469', 'no':'6470', 'invalid':'6470'}
        elif json_data['currentNodeId'] == '6471':
            prediction = dataset['cls_result']['predictions'][0]['dealml']
            out_nodes = {'yes':'6473', 'no':'6472', 'invalid':'6472'}
        elif json_data['currentNodeId'] == '6474':
            prediction = dataset['cls_result']['predictions'][0]['ask_know']
            out_nodes = {'yes':'6475', 'no':'6477', 'invalid':'6477'}
