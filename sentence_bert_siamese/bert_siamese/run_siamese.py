...
import csv
import os
import my_modeling2 as my_modeling
import optimization
import tokenization
...
def convert_single_example2(ex_index, example, label_list, max_seq_length, tokenizer):
    ...
    segment_a_ids = [0] * len(tokens_a)
    segment_b_ids = [0] * len(tokens_b)
    ...
    input_a_ids = padding_zero(input_a_ids, max_seq_length)
    input_b_ids = padding_zero(input_b_ids, max_seq_length)
    input_a_mask = padding_zero(input_a_mask, max_seq_length)
    input_b_mask = padding_zero(input_b_mask, max_seq_length)
    segment_a_ids = padding_zero(segment_a_ids, max_seq_length)
    segment_b_ids = padding_zero(segment_b_ids, max_seq_length)
...
def file_based_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        feature = convert_single_example2(
...
def create_model(
    ...
    print("input_ids" * 5, input_a_ids)
    print("input_ids" * 5, input_b_ids)
    print("input_mask" * 5, input_a_mask)
    print("input_mask" * 5, input_b_mask)
    print("segment_ids" * 5, segment_a_ids)
    print("segment_ids" * 5, segment_b_ids)
    print("labels" * 5, labels)
    print("num_labels" * 5, num_labels)
    ...
    output_layer_b = tf.reduce_max(sequence_output_b, axis=1)
    ...
    with tf.variable_scope("loss"):
        ...
        #             kernel_initializer=my_modeling.create_initializer(bert_config.initializer_range),
        #         )
        ...
        loss, sim = ClassificationLoss(output_layer_a, output_layer_b, labels, is_training)
        # loss, sim = ContrastiveLoss(output_layer_a, output_layer_b, labels)
        return (loss, sim)


def ClassificationLoss(tensor_a, tensor_b, labels, is_training=False):
    # output_layer = tf.concat([tensor_a, tensor_b, tf.abs(tensor_a - tensor_b)], axis=1)
    # output_layer = tf.concat([tensor_a, tensor_b, tensor_a * tensor_b], axis=1)
    output_layer = tf.concat([tensor_a, tensor_b, tf.abs(tensor_a - tensor_b), tensor_a * tensor_b], axis=1)
    logits = tf.layers.dense(
                    output_layer,
                    1,
                    kernel_initializer=my_modeling.create_initializer(0.02),
                )
    ...
    labels = tf.reshape(tf.to_float(labels), [-1, 1])
    per_example_loss = -(labels * tf.math.log(probabilities) + (1-labels) * tf.math.log(1-probabilities))
    loss = tf.reduce_mean(per_example_loss)
    return (loss, 1 - probabilities)


def ContrastiveLoss(tensor_a, tensor_b, labels):
    ...
    print("pooled_len_1", pooled_len_1)
    print("pooled_len_2", pooled_len_2)
    print("pooled_mul_12", pooled_mul_12)
    sim = tf.div(pooled_mul_12, pooled_len_1 * pooled_len_2 + 1e-8, name="scores")
    sim = 1 - sim
    y_pre_square = sim ** 2
    # margin_square = tf.square(tf.maximum(1-sim, 0.0))
    margin_square = tf.square(tf.maximum(1 - sim, 0.0))
    loss = tf.reduce_mean(
        tf.to_float(labels) * y_pre_square + (1.0 - tf.to_float(labels)) * margin_square
    )
    print("loss", loss)
    print("sim", sim)
    return loss, sim
...
def ContrastiveLoss2(tensor_a, tensor_b, labels):
    print("tensor_a", tensor_a)
    print("tensor_b", tensor_b)
    sub = tensor_a - tensor_b
    sim = tf.square(tf.reduce_mean(sub * sub))
    print("sim", sim)
    y_pre_square = sim ** 2
    # margin_square = tf.square(tf.maximum(1-sim, 0.0))
    margin_square = tf.square(tf.maximum(1 - sim, 0.0))
...
def model_fn_builder(
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        input_a_mask = features["input_a_mask"]
        input_b_mask = features["input_b_mask"]
        segment_a_ids = features["segment_a_ids"]
        segment_b_ids = features["segment_b_ids"]
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            output_spec = tf.contrib.tpu.TPUEstimatorSpec(
                mode=mode, loss=total_loss, train_op=train_op, scaffold_fn=scaffold_fn, training_hooks=[logging_hook]
            )
    ...
    if FLAGS.do_predict:
        ...
        with tf.gfile.GFile(output_predict_file, "w") as writer:
            ...
            for (i, prediction) in enumerate(result):
                ...
                #        for class_probability in probabilities) + "\n"
                # output_line = str(probabilities) + "\n"  # 如果是对比loss
                output_line = str(probabilities[0]) + "\n"  # 如果是分类loss
                writer.write(output_line)
                num_written_lines += 1
