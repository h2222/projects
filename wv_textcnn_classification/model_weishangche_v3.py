    ...
    def build_graph(self):
        ...
        if self.attention:
            ...
            self.attention_w = tf.Variable(
                tf.random_normal([self.embedding_size, self.attention_hidden_dim], 0.0, 1.0),
                name="attention_w")
            self.attention_u = tf.Variable(
                tf.random_normal([self.embedding_size, self.attention_hidden_dim], 0.0, 1.0),
                name="attention_u")
            self.attention_v = tf.Variable(
                tf.random_normal([self.attention_hidden_dim, 1], 0.0, 1.0),
                name="attention_v")
...
if __name__ == "__main__":
    ...
    for batch_sz in batch_ls:
        for epoch in epoch_ls:
            ...
            # 数据导入, batch
            train_ratio = 0.95
            X_no_trip = np.load("test/data_4_5_9_10.csv_no_trip_total_trans.csv_rg_p.csv__basew2v6_len-"+str(sentence_len)+"_mat.npy")
            X_trip = np.load("test/data_4_5_9_10.csv_others_total_trans.csv_rg_n.csv__basew2v6_len-"+str(sentence_len)+"_mat.npy")
            ...
            # 模型保存1(只保留pb文件)
            save_model_file = os.path.join(model.save_path, 'single_model_bz-%d_e-%d_do-%.2f_l2-%.2f_wz-%d_transvt-%s_pn1v1-%s_attn-%s_w2v6_new_rgfor2data_dataset_v2_test.pb' % (
