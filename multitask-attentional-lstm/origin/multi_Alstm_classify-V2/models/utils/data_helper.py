...
def build_corpus(seq_info, dict_type2label, config):
    ...
    #停词表 
    stopwords = pd.read_csv(config.stopwords_path,  encoding='UTF-8', sep='\n')
    ...
    for item_info in seq_info:
        ...
        if len(text) != 0:
            res_texts.append(text[:config.max_len])
...
def load_data(config, class_name):
    ...
    if config.is_train:
        ...
        curr_target_col = config.use_cols[-1]
...
def fill_feed_dict(data_X, data_Y, class_train, seq_l, data_Text_Real, batch_size):
    ...
    class_name_batch = []
    seq_l_batch = []
    for idx in range(len(data_X) // batch_size):
