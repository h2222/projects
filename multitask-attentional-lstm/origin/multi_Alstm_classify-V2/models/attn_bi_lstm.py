...
class ABLSTM(object):
    def __init__(self, config):
        ...
        with tf.name_scope("Input"):
            self.x = tf.placeholder(tf.string, [None, self.max_len], name='input_x')
    ...
    def build_graph(self):
        ...
        self.features_idx = table.lookup(self.x)
        
        # Word embedding
        embeddings_var = tf.Variable(tf.random_uniform([self.vocab_size, self.embedding_size], -1.0, 1.0),
                                     trainable=True)
        rnn_inputs = tf.nn.embedding_lookup(embeddings_var, self.features_idx)
        ...
        for item in self.cls_type:
            with tf.variable_scope(item):
                ...
                setattr(self, 'train_' + item + '_op',
                        self.optimizer.apply_gradients(zip(grads, tvars), global_step=self.global_step,
                                                       name='train_step'))
...
def run_train_step(model, sess, batch, class_name, dropout_keep_prob):
    ...
    to_return = [getattr(model, 'train_' + class_name + '_op'), model.global_step, getattr(model, 'loss_' + class_name),
                 getattr(model, 'prediction_' + class_name)]
...
def data_preprocessing_gzb(table, train, test, vocab_path, max_len):
    train_idx = []
    for item in train:
        features = tf.constant(item.split(" "))
        ids = table.lookup(features)
        train_idx.append(list(sess.run(ids)))
    test_idx = []
    for item in test:
        features = tf.constant(item.split(" "))
        ids = table.lookup(features)
        test_idx.append(list(sess.run(ids)))
...
if __name__ == '__main__':
    ...
    for epoch in range(config.train_epochs):
        ...
        for x_batch, y_batch, class_name, seq_l_batch, _ in fill_feed_dict(train_texts, train_y, class_train, seq_l_train, train_texts,
                                                              config.batch_size):
            step, loss, accuracy = run_train_step(classifier, sess, (x_batch, y_batch, seq_l_batch), class_name,
        ...
        for x_batch, y_batch, class_name, seq_l_batch, text_real_batch in fill_feed_dict(train_texts, train_y, class_train, seq_l_train, train_texts,
                                                                            config.batch_size):
        ...
        for x_batch, y_batch, class_name, seq_l_batch, text_real_batch in fill_feed_dict(test_texts, test_y, class_test, seq_l_test, test_texts,
                                                                            config.batch_size):
            result = run_eval_step(classifier, sess, class_name, (x_batch, y_batch, seq_l_batch))
            test_count += 1
        ...
        if test_f > the_best_f and epoch + 1 > config.checkpoint_from_epoch:
            ...
            save_forecast_sample(train_predictions, train_targets, train_texts_reals, dict_id2type, config.train_sample,
                                 train_classify_name, epoch)
            save_forecast_sample(test_predictions, test_targets, test_texts_reals, dict_id2type, config.test_sample,
                                 test_classify_name, epoch)
